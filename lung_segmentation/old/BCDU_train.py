...
weight_path = 'log/params/BCDUnet2.pth'
data_path = 'data/train2.csv'
#图片保存路径
save_path = r'log/BCDUnet/train'
csv_path='log/BCDUnet/training_metrics2.csv'
...
if __name__ == '__main__':
    ...
    for epoch in range(1, num_epoch + 1):   
        ...
        for i, (image, mask) in enumerate(tqdm(train_loader, desc=f'Epoch {epoch}')):
            ...
            if i%50==0:
                save_image(img, f'{save_path}/2epoch_{epoch}_train_{i}.png')
        ...
        with torch.no_grad():
            for i, (image, mask) in enumerate(tqdm(val_loader, desc='Validation')):
                ...
                img = torch.stack([_image, _mask, _out_image], dim=0)
                if i%10==0:
                    save_image(img, f'{save_path}/2epoch_{epoch}_vali_{i}.png')
                # 计算准确率
                accuracy = calculate_accuracy(out_img, mask)
        ...
        # # 超参数
        # if epoch > 1 and len(all_val_losses) >= 5 and all_val_losses[-1] > all_val_losses[-2]and all_val_losses[-1] > all_val_losses[-3]and all_val_losses[-1] > all_val_losses[-4] :
        #     # 如果验证集损失上升，减小学习率
        #     lr *= 0.9
    ...
    plt.legend()
    plt.savefig(r'log/BCDUnet/Loss99.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig(r'log/BCDUnet/Accuracy99.png')
    plt.show()
